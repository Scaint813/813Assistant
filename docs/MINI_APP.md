...
обновляет его при входе в приложение. Mini App показывает владельцу инструкцию
для личной автоматизации iPhone, URL приёмника и отдельный Bearer token.

Автоматизация запускается при закрытии HSE App, выбирает только события
календаря `HSE` на 14 дней и отправляет зеркало на сервер. Пустой корректный
снимок удаляет устаревшие пары `hse_ios`; личные календари и события других
источников не меняются. Пошаговая настройка:
